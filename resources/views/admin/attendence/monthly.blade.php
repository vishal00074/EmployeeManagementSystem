...
@php
$totalcount = DB::table('attendences')
->where('employee_id', $id)
->select('time_in', 'time_out', 'date')
->whereMonth('date', Carbon\Carbon::now('Asia/Kolkata')->month)
->count();
@endphp
<!-- Content area -->
<div class="content">
        ...
        <table class="table get_Customer_details">
            ...
            <body>
                ...
                <tr style="background-color: {{ $rowColor }}">
                    ...
                    <td>{{ $date['day'] ?? '' }}</td>
                    <td>{{ isset($attendence->time_in) ? \Carbon\Carbon::parse($attendence->time_in)->format('h:i A') : '' }}</td>
                    <td>{{ isset($attendence->time_out) ? \Carbon\Carbon::parse($attendence->time_out)->format('h:i A') : '' }}</td>
